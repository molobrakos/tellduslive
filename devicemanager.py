...
class Tellstick(object):
    ...
    def __init__(self, config_file=None, logger=None):
        ...
        for controller in self._controllers:
            ...
            t.start()
        self._sleept = threading.Thread(target=self._emptynewdevices, args=(0,))
    ...
    def _addcontroller(self, settings):
        ...
        self._controllers.append(Controller(logger=self._LOGGER))
    ...
    def controller(self, controllerId):
        ...
        Returns:
          the controller specified by `controllerId` or None of no controller was found
        """
    ...
    def _sensorhandler(self, packet, controller):
        ...
        for s in self._devices:
            if s.sensorId() == packet["sensorId"]:
                self._LOGGER.debug("Updating state for sensor %s", s.sensorId())
                s.setSensorValues(packet["data"])
                self._LOGGER.debug("Updated state for sensor %s", s.name())
        ...
        if new:
            ...
            self._adddevice({'name': "Dinamicly added sensor"+str(self._getnewid()),
                             'id': self._getnewid(), 'sensorId': packet["sensorId"],
                             'protocol': packet["protocol"], 'model': packet["model"],
    ...
    def _switchhandler(self, packet, controller):
        ...
        if new:
            if not self._sleept.isAlive():
                self._sleept = threading.Thread(target=self._emptynewdevices, args=(3, ))
                self._threads.append(self._sleept)
                self._sleept.start()
            if controller_id in self._newdevices[0]:
                if controller_id in self._newdevices[1]:
                    if controller_id in self._newdevices[2]:
                        self._LOGGER.info("Discovered new controller %s", controller_id)
                        self._LOGGER.info("newid %s", self._getnewid())
                        self._adddevice({'name': "Dinamicly added swich"+str(self._getnewid()),
                                         'id': self._getnewid(),
                                         'parameters': {'house':  packet["house"],
                                                        'unit': packet['unit']},
